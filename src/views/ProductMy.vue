...
const {addBook} = storeToRefs(useCartStore())
...
const message = ref('')
 ...
 //products.value = data.books
  products.value = fullPrice.value
...
<template>
  <input type="text" name="search" v-model="message" placeholder="Название книги" />
  <div v-if="!isLoading">
    <div class="cart">
      Добавлено к прочтению {{ addBook}} шт
      Читаю {{ planRead }} шт
      Прочитал {{ writeBook }} шт
    </div>
    <div class="catalog">
      <MyCard
